...
bg1.add(jRadioButton3);
    ...
    private void btn_executeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_executeActionPerformed
       ...
       for (i=0;i<n;i++){
        ...
        do{
             ...
             for (i=0;i<n;i++){
                 try{
                     ...
                     Double b=1/Tlast_sumi[i]+Tsumj.get(j);
                     Tsumj.set(j, b);
                 }
                 catch(Exception e){
                     int jr=0;
             ...
             if (j%100==0){
                 Double Temp1, Temp2;
                  Temp1 = Temp2 = 0.;
                  for (i=1;i<=j;i++){
                      Temp1 = Tsumj.get(i)+Temp1;
                      Temp2 += Tsumj.get(j) * Tsumj.get(i);
                  }
                  Eps = Tgamma * Math.sqrt((Math.pow(Temp1, -2) * Temp2 - 1 / j) * (j / (j - 1)));
             }
        } while (Eps <= Ez && j < m_max);
        m = j;
        T = 0;
        for (s=0;s<n;s++){
            T += 1 / Tlast_sumi[s];
        ...
        Collections.sort(Tsumj);
 //       Arrays.sort(Tsumj, 0, m);
        double  gamma1, gamma2;
        Double Tb,Tt;
        gamma1 = (1 - gamma) / 2;
        gamma2 = (1 + gamma) / 2;
        int bottom = ((Double)(m * gamma1)).intValue();
        int top = ((Double)(m * gamma2)).intValue();
        Tb = Tsumj.get(bottom);
        Tt = Tsumj.get(top);
        box_for_gamma1.setText(String.valueOf(gamma1));
        box_for_gamma2.setText(String.valueOf(gamma2));
        box_for_Tmin.setText(String.valueOf(Tsumj.get(0)));        
        Double max =Collections.max(Tsumj);
        box_for_Tmax.setText(String.valueOf(max)); 
        box_for_Tb.setText(String.valueOf(Tb)); 
        box_for_Ttop.setText (String.valueOf(Tt));
        box_M_after_ex.setText(String.valueOf(m)); 
        box_E_after_ex.setText (String.valueOf(Eps));
        Double stepT=(max - Tsumj.get(0)) / 20;
        double [] T_diagram=new double[20];
        
        T_diagram[0] = Tsumj.get(0) + stepT;
        for (i=1;i<20;i++){
